...
class Main(object):
    ...
    def _get_total_requests(self):
        ...
        for mem in Config.TestCase:
            ...
            if isinstance(mem['testcase'], (list, tuple)):
                for test_case in mem['testcase']:
                    total_request_num += nums * self._get_step_requests(test_case)
            else:
                total_request_num += nums * self._get_step_requests(mem['testcase'])
