...
class MyHTTPConnection:
    def __init__(self, host, is_secure=False, port=None, ssl_version=None, timeout=80, serial_no=0, long_connection=False,
        ...
        if self.isSecure:
            if not self.is_http2:
                if Util.compare_version(sys.version.split()[0], '2.7.9') >= 0:
                    self.connection = httplib.HTTPSConnection(self.host + ':' + self.port, timeout=self.timeout,
    ...
    def make_request(self, cal_md5=None, memory_file=None):
        ...
        try:
            ...
            if not self.defineResponse.status:
                ...
                logging.warn(self.defineResponse.status)
        except Exception, data:
            import traceback
            stack = traceback.format_exc()
            logging.error(
