...
def read_users():
    ...
    load users.dat 
    """
    global USERS, CONFIG, USER_CONFIG_FILE
    index = -1
    try:
...
def create_bucket(process_id, user, conn, result_queue):
    ...
    while i < CONFIG['BucketsPerUser']:
        if not CONFIG['BucketNameFixed']:
            rest.bucket = '%s-%s-%d' % (user.sk.lower(), CONFIG['BucketNamePrefix'], i)
        if CONFIG['TpsPerThread']:  # 限制tps
            # 按限制的tps数计算当前应该到的时间。计算方法： 当前已完成的请求 / 限制TPS + 并发开始时间
...
def put_bucket_log(process_id, user, conn, result_queue):
    ...
    if CONFIG['BucketNameFixed']:
        ...
        target_bucket = '%s-%s-%d' % (user.sk.lower(), CONFIG['BucketNamePrefix'], i)
...
def append_object(process_id, user, conn, result_queue):
    ...
    if CONFIG['GetPositionFromMeta']:
        ...
        for i in range_arr:
            if not CONFIG['BucketNameFixed']:
                rest_head.bucket = '%s-%s-%d' % (user.sk.lower(), CONFIG['BucketNamePrefix'], i)
                rest_append.bucket = '%s-%s-%d' % (user.sk.lower(), CONFIG['BucketNamePrefix'], i)
            j = 0
            while j < CONFIG['ObjectsPerBucketPerThread']:
                if not CONFIG['ObjectNameFixed']:
                    if CONFIG['ObjectLexical']:
...
def start_process(process_id, user, test_case, results_queue, valid_start_time, valid_end_time, current_threads, lock,
    ...
    if not conn:
        ...
        conn = obsPyCmd.MyHTTPConnection(host=CONFIG['OSCs'], is_secure=CONFIG['IsHTTPs'], port=port,
    ...
    for end_point in oscs:
        ...
        try:
            if CONFIG['IsHTTPs']:
                ...
                port = CONFIG['HTTPPort']
            test_conn = obsPyCmd.MyHTTPConnection(host=end_point, is_secure=CONFIG['IsHTTPs'], port=port,
...
def run_in_integrated_mode(mode):
    ...
    results_writer = results.ResultWriter(CONFIG, TESTCASES[CONFIG['Testcase']].split(';')[0].split(';')[0],
                                          ...
                                          valid_start_time, valid_end_time, current_threads, RESULT_PATH)
...
def param_analysis():
    ...
    parser.add_option("-f", "--config_file",
                      ...
                      dest="config_file",
                      default=None,
                      help='Required:False   Type:str   Help:config file')

    # 设置用户配置文件
    parser.add_option("-u", "--user_config",
                      type="str",
                      dest="user_config",
                      default=None,
                      help='Required:False   Type:str   Help:user config file')
    ...
    parser.add_option("-t", "--testcase",
                      ...
                      dest='testcase',
                      default=None,
                      help='Required:False   Type:int   Help:testcase number')

    options, args = parser.parse_args()
    config_file = options.config_file
    user_config_file = options.user_config
    output_dir = options.output_dir
    testcase_num = options.testcase
    return config_file, user_config_file, output_dir, testcase_num


def set_log(log_path):
    if not os.path.exists(log_path):
        os.mkdir(log_path)
    log_name = os.path.join(log_path, "obscmdbench.log")
    logger = logging.getLogger()
    handler = logging.handlers.ConcurrentRotatingFileHandler(log_name, "a", 100 * 1024 * 1024, 30)
    logger.setLevel(logging.INFO)
    formatter = logging.Formatter('[%(asctime)s] [%(process)d] [%(levelname)s] [%(filename)s:%(lineno)s] %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)


if __name__ == '__main__':
    global USER_CONFIG_FILE, RESULT_PATH
    # 创建log目录，读取log配置
    ...
    # 解析参数
    config_file, user_config_file, output_dir, testcase_num = param_analysis()
    # 设置用户配置文件
    if user_config_file is None:
        USER_CONFIG_FILE = './users.dat'
    else:
